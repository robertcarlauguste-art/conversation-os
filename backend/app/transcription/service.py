...
from app.transcription.events import emit_transcription_completed, emit_transcription_failed
...
class TranscriptionService(BaseService[TranscriptRepository]):
    ...
    async def transcribe(
        self, *, conversation_id: uuid.UUID, storage_path: str, filename: str
    ) -> Transcript:
        start = time.perf_counter()
        logger.info("transcription_started conversation_id=%s", conversation_id)
        ...
        try:
            ...
            result = await self._provider.transcribe(audio_bytes=audio_bytes, filename=filename)
            ...
            emit_transcription_completed(conversation_id, transcript.id)
            ...
            emit_transcription_failed(conversation_id, str(exc))
            duration_ms = (time.perf_counter() - start) * 1000
            logger.warning(
